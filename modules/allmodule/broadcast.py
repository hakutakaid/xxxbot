#Credit By : Zaid-Userbot
import asyncio
from pyrogram import Client, enums, filters
from pyrogram import *
from pyrogram.types import Message
from pyrogram.types import *
from modules.config import DEVS, cmd
from modules.config import BL_GCAST as BL
...
async def gcast_cmd(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        tex = await message.reply_text("`Started global broadcast...`", quote=True)  # Change this line
    else:
        return await message.reply_text("**Give A Message or Reply**", quote=True)
    done = 0
    error = 0
...
async def gucast(client: Client, message: Message):
    if message.reply_to_message or get_arg(message):
        tex = await message.edit_text("`Started global broadcast...`")  # Change this line
    else:
